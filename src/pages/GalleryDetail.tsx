...
const GalleryDetail = () => {
    ...
    const fetchItem = async () => {
      ...
      try {
        ...
        const {
          data,
          error: fetchError
        } = await client.from('design_assets').select('*').eq('id', id).eq('is_active', true).maybeSingle();
        if (fetchError) {
        ...
        const {
          data: relatedData
        } = await client.from('design_assets').select('*').eq('is_active', true).neq('id', id).limit(4);
        if (relatedData) {
          // Filter related items by matching style or sculpture type
          const parsedRelated = (relatedData as DesignAsset[]).map(parseDesignAsset).filter(r => r.sculptureType === parsedItem.sculptureType || r.style === parsedItem.style).slice(0, 4);
  ...
  return <div className="min-h-screen bg-background font-sans">
            ...
            <main className="pt-32 pb-16">
                <div className="container mx-auto px-6">
                        ...
                        <div className="relative">
                            <div className="sticky top-32">
                                <div className="w-full bg-gray-100 overflow-hidden shadow-lg flex items-center justify-center">
                                    <img src={item.imageUrl} alt={item.imageAlt} className="w-full h-auto object-contain max-h-[70vh]" onError={e => {
                  (e.target as HTMLImageElement).src = 'https://placehold.co/600x400?text=Image+Unavailable';
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            {item.description && <div className="prose prose-lg text-muted-foreground mb-8 leading-relaxed">
                ...
                {/* Recommendations */}
                {relatedItems.length > 0 && <section className="mt-32 bg-luxury-cream/20 py-20">
                        <div className="container mx-auto px-6">
                            <h2 className="heading-md text-3xl text-center mb-12">You May Also Like</h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                                {relatedItems.map(related => <GalleryItem key={related.id} item={{
              id: related.id,
              title: related.title,
